...
impl Pipe {
	...
	pub fn update(&mut self, program: &mut process::Child,
		progress: &mut cli::Progress) -> ()
	{
		{
			let mut output = BufReader::new(
				program.stdout.as_mut().unwrap());
...
pub fn wait_on(mut program: process::Child, mut progress: Option<cli::Progress>,
	mut pipe: Pipe, error: &str)
{
	loop {
