...
pub fn execute(cargo_toml: parse::CargoToml, translations: resources::Lang) {
    ...
    // Build the program
    {
        program::execute_log("cargo", vec!["build", "--release", "--bin", "gsp_program"]);
    }

    // App Domain
    let app_domain = format!("{}.{}", &domain(&cargo_toml), &cargo_toml.subdomain).to_string();

    // Copy Executable Into FlatPak
    {
        file::copy(
            "target/release/gsp_program",
    ...
    // Create .desktop
    {
        let mut desktop_data = format!(
            "\
             ...
             Icon={}\n\
             Terminal=false\n",
            &cargo_toml.subdomain, &cargo_toml.subdomain, &app_domain
        );

        // localize / english
        if let Some(ref en) = translations.en {
            desktop_data.push_str(&format!("Name[en]={}\n", en.0 .0));
            desktop_data.push_str(&format!("Comment={}\nComment[en]={}\n", en.0 .1, en.0 .1));
        // TODO: Dialects
        } else {
